...
class LLMClient:
    """Generic LLM Client for OpenRouter (can extend for other providers)."""
    ...
    def analyze_logs(self, csv_text: str, temperature: float = 0.3):
        """
        Send entire CSV log to LLM for analysis.
        Expect LLM to return a JSON array with analysis for each row.
        """
        prompt = f"""
