...
# CRITICAL FIX: unquote_plus handles 'union+select' -> 'union select'
...
class LogEnricher:
    ...
    def enrich_event(self, event: WebLogEvent) -> dict:
        ...
        country = "XX"
        if self.geo_reader and event.source_ip:
            try:
                res = self.geo_reader.city(event.source_ip)
...
class OwaspRuleEngine:
    def __init__(self):
        self.sqli = [re.compile(p, re.I) for p in [
            r"union\s+(all\s+)?select",
            r"select\s+.*\s+from",
            r"insert\s+into\s+.*\s+values",
            r"drop\s+(table|database)",
            r"waitfor\s+delay",  # Specific CSIC pattern
            r"exec(\s|\+)+(xp|sp)_",  # MSSQL Exec
            r"(or|and)\s+['0-9]+=['0-9]+",
            r"--",
            r";\s*$"
        ]]

        self.xss = [re.compile(p, re.I) for p in [
            r"<script",
            r"javascript:",
            r"on\w+\s*=",
            r"alert\(",
        ...
        self.traversal = [re.compile(p, re.I) for p in [
            r"\.\./",
            r"%2e%2e%2f",
            r"/etc/passwd",
            r"c:\\windows",
    ...
    def evaluate(self, enriched: dict) -> dict:
        # Aggressive: No whitelisting. Check everything.
        content = enriched['path'] + " " + enriched['full_payload']
        return {
            "is_sqli": any(r.search(content) for r in self.sqli),
            "is_xss": any(r.search(content) for r in self.xss),
            "is_traversal": any(r.search(content) for r in self.traversal)
        }
    ...
    def process_event(self, enriched: dict, rules: dict):
        ...
        if key in self.active:
            ...
            if is_idle or is_too_long:
                yield s
                self._create(key, enriched, rules)  # Reset session
            else:
